...
@WebFilter(
	...
	urlPatterns={"/ch08/DriverManagerServlet", "/ch08/DataSourceServlet"},
	initParams = {
		@WebInitParam(
			name="prefix", 
			value="程式執行效能觀測:"
...
public class PerformanceFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
        ...
        System.out.println("target=" + target + ", req.getServletPath()=" + req.getServletPath());
        if ( target!= null && target.equals("DM") ) {
        	session.setAttribute("DM","Driver Manager 版，時間=" + (end - begin) + " 毫秒(ms)" );
        }
        if ( target!= null &&  target.equals("DS") ) {
